    ...
    return (
        <div className={`flex flex-col h-full font-sans transition-all duration-300 bg-secondary border-r border-border animate-fade-in relative overflow-hidden ${isSidebarCollapsed ? 'items-center' : ''}`}>
            ...
            <div
                ...
                className={`h-14 flex items-center border-b border-border/40 mb-2 bg-background/20 backdrop-blur-sm shrink-0 shadow-sm transition-all ${isSidebarCollapsed ? 'justify-center cursor-pointer hover:bg-background/40' : 'justify-between px-4'}`}
            >
                                    ...
                                    {({ isActive }) => (
                                        <>
                                            ...
                                            <div className="flex items-center gap-3">
                                                <span className={`transition-all duration-300 ${isActive ? 'text-primary scale-110' : 'group-hover:text-foreground'}`}>{item.icon}</span>
                                                {!isSidebarCollapsed && (
            ...
            <div className={`mt-auto ${isSidebarCollapsed ? 'p-2' : 'p-4'}`}>
                <div className={`rounded-xl border border-border bg-card flex items-center ${isSidebarCollapsed ? 'justify-center p-2' : 'justify-between p-3'} group cursor-pointer shadow-sm hover:border-primary/20 transition-all`}>
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <div className="w-9 h-9 rounded-lg bg-secondary border border-border flex items-center justify-center overflow-hidden">
                                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-tighter">AD</span>
                            </div>
                            <div className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 border-2 rounded-full bg-emerald-500 border-card"></div>
                        </div>
                        {!isSidebarCollapsed && (
                            <div className="overflow-hidden">
                                <div className="text-[12px] font-black truncate text-foreground uppercase tracking-tight">System Admin</div>
                                <div className="text-[9px] font-bold truncate text-muted-foreground uppercase tracking-wider">Sargodha HQ</div>
                            </div>
                        )}
                    </div>
                    {!isSidebarCollapsed && (
                        <div className="p-2 rounded-lg transition-all shrink-0 text-muted-foreground hover:text-rose-500 hover:bg-rose-500/10">
                            <LogOut size={16} />
                        </div>
                    )}
                </div>
